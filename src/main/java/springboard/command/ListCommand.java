...
@Service
	...
	Service 객체는 Controller와 Model사이의에서 중재역할을 함.
*/
@Service
public class ListCommand implements BbsCommandImpl{
	...
	/*
	컨트롤러에서 인자로 전달해준 model객체를 매개변수로 전달 받는다.
	...
	*/
	...
	public void execute(Model model) {
		...
		/*
		컨트롤러에서 넘겨준파라미터를 asMap() 메소드를 통해 
		Map컬렉션으로 변환한다. 그리고 request객체를 형변환하여 가져온다.
		*/
		...
		for(SpringBbsDTO row : listRows) {
			...
			//답변글에 대한 리스트 처리(re3.gif 이미지를 제목에 삽입)
			String reSpace = "";
			//해당 게시물의 indent가 0보다 크면(답변글이라면)
			if(row.getBindent() > 0) {
				//indent의 크기만큼 공백을 추가해준다
				for(int i=0; i<row.getBindent(); i++) {
					reSpace += "&nbsp;&nbsp;";
				}
				//reply이미지를 추가한다
				row.setTitle(reSpace+
						"<img src='../resources/re3.gif'>"
						+ row.getTitle());
			}
		}
		//출력리스트에 저장
